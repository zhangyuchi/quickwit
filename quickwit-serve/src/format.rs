...
#[derive(Serialize)]
pub(crate) struct FormatError<'a> {
    #[serde(skip_serializing)]
    pub code: ServiceErrorCode,
    pub error: &'a str,
}

impl<'a> ToString for FormatError<'a> {
    fn to_string(&self) -> String {
        self.error.to_string()
    }
}

impl<'a> ServiceError for FormatError<'a> {
...
impl Format {
    ...
    pub(crate) fn make_reply_for_err<E: crate::error::ServiceError>(
        &self,
        err: E,
    ...
    pub(crate) fn make_reply<T: serde::Serialize, E: crate::error::ServiceError>(
        self,
        result: Result<T, E>,
    ) -> WithStatus<WithHeader<String>> {
        match result {
