...
impl Queues {
    ...
    // TODO use a tx per `DocBatch`.
    pub fn append(&mut self, queue_id: &str, record: &[u8]) -> crate::Result<()> {
        ...
        self.db
            .put_cf_opt(&cf_ref, next_position.as_ref(), record, &write_options)?;
    ...
    fn test_create_multiple_queue() {
        ...
        let records: Vec<Record> = record_queue_ids
            ...
            .map(|queue_id| {
                ...
                let payload: Vec<u8> = repeat_with(|| rand::random::<u8>())
                    .take(num_bytes)
                    .collect();
                Record {
                    queue_id: queue_id.to_string(),
