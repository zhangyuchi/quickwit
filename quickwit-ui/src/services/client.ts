...
import { Index, IndexMetadata, MemberList, SearchRequest, SearchResponse } from "../utils/models";
...
export class Client {
  ...
  async getIndex(indexId: string): Promise<Index> {
    const index_metadata: IndexMetadata = await this.fetch(`${this.apiRoot()}indexes/${indexId}`, {});
  ...
  async listIndexes(): Promise<Array<IndexMetadata>> {
    return await this.fetch(`${this.apiRoot()}indexes`, {});
  }
  ...
  async fetch<T>(url: string, params: RequestInit): Promise<T> {
    try {
      ...
      return await Promise.reject({
        message: `Error in response: status=${response.status}, text=${response.text}`
      });
    } catch (error) {
      return await Promise.reject({
